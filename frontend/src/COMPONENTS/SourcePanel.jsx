function SourcePanel({ sources }) {
  return (
    <div style={styles.panel}>
      <h3 style={styles.title}>📌 Sources consultées</h3>
      {sources && sources.length > 0 ? (
        <div style={styles.sourceList}>
          {sources.map((src, index) => (
            <div key={index} style={styles.sourceCard}>
              <span style={styles.badge}>Extrait n°{index + 1}</span>
        ...
        <p style={styles.empty}>Les extraits du PDF utilisés par l'IA s'afficheront ici.</p>
      )}
    </div>
  );
}
